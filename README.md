...
- The landing-page 3D scene renders at full desktop detail on every device — identical geometry, the full 14,000-star field, antialiasing on, and the same pixel-ratio cap (2). The only mobile-specific reduction is procedural texture resolution (2048² vs desktop's 4096²): visually identical at phone render sizes, but ~4x less GPU memory, which keeps it clear of the iOS Safari out-of-memory limit. Combined with the visibility-skip below, this keeps mobile both detailed and smooth.
